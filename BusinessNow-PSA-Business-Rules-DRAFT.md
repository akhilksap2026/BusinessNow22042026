...
BR-TIME-026: The copy-last-week function recreates the previous week's time entries in the current week, mapped to the corresponding day of the new week. Entries from a locked week (an approved or closed period) are not copied.

BR-TIME-027: The import-from-allocations function pre-populates a draft timesheet with time entries based on the team member's active allocations for that week. The user reviews and edits these entries before submitting.

#### 3.7.8 Integration Points
...
**Total Business Rules Defined:** 193
