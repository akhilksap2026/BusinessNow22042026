...
### 2026-04-24 — Task #1: AI Time Tracking Assistant (addon)

Added a floating bottom-right AI chatbot that helps users log time via natural language. **Pure addon** — no existing routes, schemas, pages, or governance touched.

**Backend (3 files, additive only)**
- `artifacts/api-server/src/lib/aiTimeAssistant.ts` (NEW) — LLM orchestrator. Uses OpenAI SDK pointed at `https://api.groq.com/openai/v1` with model `llama-3.3-70b-versatile` (`GROQ_API_KEY`); falls back to `gpt-4o-mini` if only `OPENAI_API_KEY` is set. Tools: `list_my_projects`, `list_my_tasks`, `list_my_time_logs`, `get_time_summary`, `propose_time_entries`. In-memory session map (max 200, 1h TTL, 40-message cap per session). The LLM never sees the userId — it is always taken from the verified `x-user-id` header.
- `artifacts/api-server/src/routes/aiChat.ts` (NEW) — `POST /api/ai-time-helper` handler. Two modes:
  - `{ sessionId, message }` → run an LLM turn, return `{ reply, proposal? }`.
  - `{ sessionId, confirmedEntries[] }` → commit each entry by calling the existing `POST /api/time-entries` via internal fetch with the caller's headers, so all governance (parent-task ban, billable cascade, lock-on-approval, invoice protection, RBAC) stays in force untouched.
  - Also accepts `reset: true` to drop server-side session history.
- `artifacts/api-server/src/routes/index.ts` — two-line addition: import `aiChatRouter`, mount with `router.use(aiChatRouter)`.
- `artifacts/api-server/package.json` — added `openai` dependency.

**Frontend (2 files, additive only)**
- `artifacts/businessnow/src/components/ai-time-assistant.tsx` (NEW) — Self-contained floating widget using existing shadcn primitives (Button, Card, ScrollArea, Badge). Collapsed: pill launcher with gradient + Sparkles icon. Expanded: 380×560 panel on desktop, full-screen sheet on mobile (`<sm`). Maintains client-side conversation history; `sessionId` persisted in `sessionStorage`. When server returns a `proposal`, renders an inline confirmation card with Confirm/Cancel buttons; on confirm, sends `confirmedEntries` back to the same endpoint and invalidates time-entries / time-summary / timesheets React Query caches so the existing Time page reflects new entries instantly.
- `artifacts/businessnow/src/App.tsx` — two-line addition: import `AITimeAssistant`, render `<AITimeAssistant />` inside `<WouterRouter>` next to `<RoleSelectorModal />`. Component renders `null` while `currentUser` is undefined so it stays hidden during `/me` bootstrap.

**Smoke tests (Groq llama-3.3-70b, all green)**
- `POST /api/ai-time-helper` with no `x-user-id` → 401 "Authentication required".
- `"What did I log in the last 7 days?"` → AI calls `list_my_time_logs`, returns a clean human summary (1 tool call).
- `"Log 2 hours today on any project for code review"` → AI calls `list_my_projects` + `list_my_tasks`, asks a clarifying question because the project name is ambiguous (correct defensive behavior).
- `"Log 0.25 hours today on the FrostLine WMS Implementation project. No specific task — log it at the project level."` → AI returns a `proposal` with one entry. Confirming via `confirmedEntries` triggers the existing `POST /api/time-entries`, which inserts row id 135 and returns `{ reply: "Logged 1 entry (0.25h total). What's next?", logged: [...] }`.

**Files removed**: none. **Existing files modified**: only `App.tsx` (2 lines) and `routes/index.ts` (2 lines). The chatbot can be removed by deleting the 2 new backend files + 1 new frontend file and reverting those 4 added lines.

**Code-review fix (same day)** — The architect flagged that the in-memory session map was keyed only by client-supplied `sessionId`, allowing cross-user chat-history leakage if a sessionId was guessed/shared. Fixed by partitioning the session store key as `${userId}:${sessionId}` and storing `ownerUserId` on each session record (defence-in-depth: the read path resets the session if `ownerUserId !== authenticatedUserId`). Regression test confirmed: user 5 reusing user 1's `sessionId` ("shared-id") gets a fresh conversation with no access to prior turns.
