...
- **Contracts entity (Apr 2026):** Added `contracts` table (`lib/db/src/schema/contracts.ts`) with FK to projects (cascade), name/status/dates/value/documentUrl/notes. New router `artifacts/api-server/src/routes/contracts.ts` exposes `GET /api/contracts`, `POST /api/contracts` (Finance: admin/PM), `PATCH /api/contracts/:id` (Finance), `DELETE /api/contracts/:id` (PM+). Finance page now has a **Contracts** tab with a full table (Name/Project/Status/Value/Start/End/Doc Link + edit/delete dropdown) and the previously-fake "New Contract" dialog now POSTs to the real endpoint, supports edit, status select (Draft/Pending Signature/Active/Expired/Terminated), Document URL field, and invalidates the contracts query on save. Search box also filters contracts by name/status/project/notes.
- **Change Order FK hardening (Apr 2026):** Added missing `.references(() => projectsTable.id, { onDelete: "cascade" })` to `change_orders.project_id` in `lib/db/src/schema/changeOrders.ts` (constraint `change_orders_project_id_projects_id_fk` confirmed in pg_constraint). The CR dialog in `project-detail.tsx` was already structurally correct (no Project Name/ID inputs — projectId comes from the route param), and the API `POST /projects/:id/change-orders` already takes projectId from the URL param exclusively, ignoring any body field. Added a project-existence check in the POST handler so unknown project IDs return a clean `404 "Project N does not exist"` instead of a Postgres FK violation surfaced as 500.
- **Undoable mutation hook (Apr 2026):** New `artifacts/businessnow/src/hooks/use-undoable-mutation.tsx` exposes `useUndoableMutation().run({ do, undo, successTitle, description, ...})` — runs the forward action, then shows a toast with an "Undo" button that fires the inverse action when clicked (idempotent: only fires once even if double-clicked). Refactored `handleDelete` (already had bespoke undo) and added undo to `handleStatusCycle` in `project-phases.tsx`, plus to `handleUpdateField` in `task-detail-sheet.tsx` (snapshots previous field value via `task[field]`). Drag-and-drop reorder in project-phases.tsx intentionally still uses its existing batched `pendingChanges` + Cmd+Z paradigm (Save/Discard buttons), which is a different UX from per-mutation undo and shouldn't be merged.
- **Budget Ledger SOW prompt (Apr 2026):** Most of the wiring (schema, auto-insert on CR approval, `GET /projects/:id/budget-entries` with running totals, Budget History card with columns Date/Type/Description/Amount/Hours/Running Total/Doc, Total row) was already in place from a prior bundle. Bug-fix bundle item 7 added the missing pieces: (a) `POST /projects/:id/budget-entries` now also accepts `type:"SOW"` (previously rejected anything but `Adjustment`); enforces single-SOW-per-project via a pre-insert check that returns a friendly 409, **plus a partial unique index `budget_entries_sow_per_project_uq` on `(project_id) WHERE type='SOW'` as the race-safe DB backstop** (concurrent POSTs are caught as PG `23505` and translated to 409; verified by firing two parallel SOW POSTs — exactly one row landed, the loser got a clean 409). The `change_order_id` unique index doesn't cover SOW rows because they have null `change_order_id`. (b) Budget History empty state now shows "No budget entries yet. Add the original SOW to start tracking." with an "Add SOW Entry" button (PM only) that opens the dialog pre-filled with `type:"SOW"`, `description:"Original SOW"`, and amount/hours from `project.budget`/`project.budgetedHours`; (c) the dialog title and description toggle on `budgetEntryForm.type` so SOW vs Adjustment look distinct; (d) Revised Budget summary card now correctly falls back to `project.budget` when `entries.length === 0` (the previous `?? Number(project.budget)` was a no-op because `totalAmount` is always defined and equals 0 for empty ledgers, which displayed $0 instead of the project budget); (e) OpenAPI spec for `POST /projects/{id}/budget-entries` updated — summary now mentions SOW + Adjustment, description documents the partial unique index, and the 409 response is declared.
...
- Removed 1 unreferenced top-level component: `apply-template-segment-modal.tsx`.
- **Fixed Account #N display bug** on /projects: `Project` schema in `lib/api-spec/openapi.yaml` was missing `companyName`, `accountDomain`, `ownerName`; Zod was stripping them on response parse. Added all three as `["string","null"]` and regenerated the client.
- **Known limitations left in place** (pre-existing, unrelated to user request): 62 typecheck errors in api-server (mostly `req.query` shape mismatches across routes), 21 in businessnow (`admin.tsx` user-property names, `project-detail.tsx` queryKey missing); /admin page console 401/403s because the seed has no `account_admin` user; auto-discovered duplicate workflows (`artifacts/api-server: API Server`, `artifacts/businessnow: web`) cannot be removed from `.replit` (auto-generated from package.json dev scripts).
...
### 2026-04-26 — 9-Item Bug Fixes & Status Unification
- **Item 1 (Task dep labels)**: Verified the original `task-detail-sheet.tsx` semantics were already correct (viewing task B, dep A→B shows "Predecessor: A"). Tightened wording: `"Successor / Blocks:"` → `"Blocks:"` for clarity. The dep label now reads `Predecessor: <name>` when viewing the successor and `Blocks: <name>` when viewing the predecessor.
- **Item 8 (Task status unification)**: Created `artifacts/businessnow/src/lib/task-status.ts` as the single source of truth with `TASK_STATUS_VALUES = ["Not Started", "In Progress", "On Hold", "Completed", "Canceled"]`. DB migrated `Blocked` → `On Hold`. Updated zod schema in `project-phases.tsx`, status dropdowns, status cycle order (`Not Started → In Progress → Completed → On Hold`), kanban columns (now 5 incl. Canceled in `project-detail.tsx`), gantt legend (`On Hold` amber + `Canceled` slate). Added `On Hold` (amber) and `Canceled` (slate) to `status-badge.tsx`; legacy `Blocked` is mapped through for any stale data.
- **Item 9 (Undo task delete)**: `project-phases.tsx handleDelete` now snapshots the original task and shows a 5-second toast with an `Undo` action button. Restore re-creates the task with full fidelity (`status, priority, assigneeIds, startDate, dueDate, effort, plannedHours, estimateHours, billable, isMilestone, isPhase, parentTaskId, sortOrder`) via `useCreateTask`. Note: hard delete means the restored task gets a new `id`, which is a known trade-off.
- **Items 2/3/4/5/6/7**: Verified to be no-op (already working / not applicable):
  - #2 Template editor expand/collapse: recursion code is correct; no template tasks have parents in seed data so the bug couldn't have been observed.
  - #3 Rate Cards admin tab: route loads for current PM role (verified by URL navigation).
  - #4 Finance project search: tested code path, `pName.toLowerCase().includes(search)` correctly resolves through `useListProjects()`.
...
### Pending Timesheet Gate — Log Time Assistant (April 27, 2026)
Added a constructive pre-flight gate to the Log Time Assistant so users see their own unresolved prior weeks before adding new entries. Strictly additive — no existing wizard step or behavior was removed.

- **Backend** — `routes/aiTimeAssistant.ts` adds `GET /api/ai/pending-timesheet-audit?userId=`. Self-only by default; `account_admin` and `super_user` may audit other users. Computes the current week start from `time_settings.weekStartDay`, then returns every prior-week timesheet with `status in (Draft|Submitted|Rejected)` and `totalHours > 0`. Each blocker carries `weekStart`, `weekEnd`, `status`, `totalHours`, `rejectionNote`, and `isHardBlocker` (Draft/Rejected = hard, Submitted = informational). Top-level `hasPendingBlockers` is true only when at least one hard blocker exists.
- **Frontend — TimeLogAssistant** (`components/time-log-assistant.tsx`) gains `STEPS.PENDING_AUDIT = -1` and runs the audit on every modal open. While loading, the wizard sits on the audit screen; when results arrive without hard blockers it auto-advances to MODE. Hard blockers are listed with status-coded badges (Rejected = red, Draft = amber, Submitted = slate) and a per-row "Resolve this week" deep-link. The "Log Time" CTA is disabled until hard blockers clear; "Refresh" re-runs the audit. Tone is encouragement-first ("Let's get you caught up first"). Network/auth failures show a Refresh + "Continue anyway" pair so users are never blocked by a flaky call. The progress bar and back button are suppressed on the audit step.
- **Frontend — Deep-link routing** — `TimesheetGrid` now accepts an optional `initialWeekStart` prop and seeds its current-week state from it on mount. `pages/time.tsx` reads `?weekStart=YYYY-MM-DD` from the URL, validates the format, passes it into the grid, and force-switches the Tabs to `timesheet` so the Resolve buttons land the user directly on the offending week (Tabs converted from uncontrolled `defaultValue` to a controlled `value`/`onValueChange` pair).
- **Coverage with existing rules** — The grid's `getBlockingPriorWeek()` (line 218) is unchanged and continues to enforce the hard rule on direct date selection inside the grid; the new audit is a higher-level nudge that surfaces the same Draft/Rejected blockers earlier in the flow.
